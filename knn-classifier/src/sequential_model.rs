...
impl KNNModel for SequentialModel {
    ...
    fn from_directory(directory: &str) -> Result<SequentialModel, String> {
        ...
        let mut new_model = SequentialModel::new();
    ...
    fn classify(&self, image: &MNISTImage, k: u32) -> Result<u8, String> {
        ...
        let mut distances: Vec<(f64, u8)> = self.dataset.iter().map(|x| (image.calculate_distance(&x.image), x.class)).collect();

        distances.sort_by(|a, b| Self::float_compare(a.0, b.0));
